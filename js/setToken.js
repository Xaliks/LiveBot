...
function validateToken(token = "") {
	...
	let invalidChars = [" ", "\t", "\r", "\n"];
	if (token.split("").filter((c) => invalidChars.includes(c)).length > 0) return [true, "TOKEN-WHITESPACE"];

	if (token.replace(/\w|-|_|\./gm, "").length > 0) return [true, "INVALID-TOKEN-CHARACTERS"];
