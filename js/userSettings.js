...
let settings = {
	...
	get settings() {
		...
		Object.keys(settings.tokenSettings).forEach((token) =>
			!settings.tokens.includes(token) ? settings.tokens.push(token) : undefined,
		);
	...
	delete(object, path = "") {
		let recursive = (settingsO, p = "") => {
			...
			for (let setting in settings) {
				if (
					JSON.stringify(settings[setting]) === JSON.stringify(object) &&
	...
	functions: {
		getGroup: (groupName) => {
			let element = Array.from(document.getElementsByClassName("settingLabel")).find(
				(e) => e.innerText === groupName,
			).parentElement;
	...
	jsonObj.settings.forEach((group) => {
		...
		if (group.update) {
			...
			btn.addEventListener("click", (event) => {
				let allDropdowns = Array.from(btn.parentElement.querySelectorAll(".dropdown")).map(
					(x) => `'${x.firstElementChild.firstElementChild.innerText.replace(/\\*'/g, "\\'")}'`,
				);
...
function genDropDown(parent, options, defaultOpt = 0, group, optionObj) {
	...
	dropdown.addEventListener("focusout", (event) => {
		if (!dropdown.contains(event.relatedTarget)) dropdown.classList.remove("openDrop");
	});
