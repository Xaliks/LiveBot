	...
	document.addEventListener("mousedown", async (e) => {
		...
		if (e.button === 0) {
			...
			if (!target.classList.contains(domElements[1])) {
				let [element] = document.getElementsByClassName("memberMenu");
			...
			if (e.target.classList.contains("rcOption") || e.target.parentElement.classList.contains("rcOption")) {
...
async function buildMsgMenu(target) {
	...
	// Check permissions
	let editGreyed = message.author.id !== bot.user.id;
	let pinGreyed = message.guild
		? Boolean(message.member.permissionsIn(message.channel).has(Discord.PermissionFlagsBits.ManageMessages))
		: false;
	let deleteGreyed =
