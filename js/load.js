...
let load = async (token) => {
	...
	bot.on("ready", async () => {
		...
		async function continueLoad() {
			...
			document.getElementById("userCardIcon").src = `${bot.user.displayAvatarURL({ size: 64 })}`;
	...
	bot.on("messageUpdate", (oldM, m) => {
		...
		if (message) message.innerHTML = `${parseMessage(m.cleanContent)} <time class='edited'>(edited)</time>`;
	});
	...
	bot.on("messageCreate", (m) => {
		...
		async function fetchLast() {
			...
			if (!previousMessage) {
				await m.channel.messages.fetch({ limit: 2 }).then((messages) => (previousMessage = messages.at(1)));
			}
