						...
						if (Number.isNaN(num)) command("The value specified must be a number", 5000);
							...
							(async () => {
								let delet = async function (tries = 0, messages = 1) {
									...
									let err = undefined;
									await selectedChan.bulkDelete(messages).catch((e) => {
										err = e;
									});
									...
									if (err?.message === "You can only bulk delete messages that are under 14 days old.") {
										...
										let delet = async function (id = undefined) {
											...
											if (err) {
												...
												delet("");
											} else command("Deleted 1 message older than 14 days", 3000);
										};
										if (args[1] === "true") {
									...
									else command(`Deleted ${messages} ${messages === 1 ? "message" : "messages"}`, 5000);
								};
